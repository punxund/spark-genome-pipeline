#!/usr/bin/env python3
"""
Spark 유전체 분석 파이프라인 - Hybrid 버전 (Python 라이브러리 활용)

이 파이프라인은 다음 단계들을 순차적으로 실행합니다:
1. 전처리 (fastp): FASTQ 파일 정리
2. 읽기 매핑 (BWA): 참조 게놈에 정렬
3. SAM 처리 (pysam): 정렬 및 통계 생성
4. 커버리지 계산 (pybedtools + pybigwig): 게놈 커버리지 분석
"""
...
class HybridGenomeAnalysisPipeline:
    """Spark 유전체 분석 파이프라인 - Hybrid 버전 클래스"""
    ...
    def _check_dependencies(self):
        """필요한 도구들과 Python 라이브러리들의 사용 가능 여부를 확인합니다."""
        logger.info("의존성 도구 및 Python 라이브러리 확인 중...")
        
        # 외부 도구들 (fastp, BWA는 여전히 필요)
        external_tools = {
        ...
        if missing_tools:
            logger.error(f"다음 외부 도구들이 설치되지 않았습니다: {', '.join(missing_tools)}")
        ...
        try:
            import pysam
            logger.info("✓ pysam 라이브러리 사용 가능")
        except ImportError:
            logger.error("pysam 라이브러리가 설치되지 않았습니다.")
            logger.error("pip install pysam으로 설치하세요.")
            sys.exit(1)
        
        try:
            import pybedtools
            logger.info("✓ pybedtools 라이브러리 사용 가능")
        except ImportError:
            logger.error("pybedtools 라이브러리가 설치되지 않았습니다.")
            logger.error("pip install pybedtools로 설치하세요.")
            sys.exit(1)
        
        try:
            import pyBigWig
            logger.info("✓ pybigwig 라이브러리 사용 가능")
        except ImportError:
            logger.error("pybigwig 라이브러리가 설치되지 않았습니다.")
            logger.error("pip install pybigwig로 설치하세요.")
            sys.exit(1)
    ...
    def run_pipeline(self, reads_dir: Path = None, reference_genome: Path = None, 
        ...
        logger.info("=" * 80)
        logger.info("Spark 유전체 분석 파이프라인 - Hybrid 버전 시작")
        logger.info("=" * 80)
        logger.info("사용 기술:")
        ...
        try:
            ...
            success_count = sum(1 for row in coverage_results if row["coverage_result"]["status"] == "success")
            ...
            logger.info("\n" + "=" * 80)
            logger.info("Spark 유전체 분석 파이프라인 - Hybrid 버전 완료")
...
def create_spark_session() -> SparkSession:
    ...
    spark = SparkSession.builder \
        .appName(f"{spark_config['spark.app.name']}-hybrid") \
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description="Spark 유전체 분석 파이프라인 - Hybrid 버전")
    ...
    try:
        ...
        if results["pipeline"]["status"] == "completed":
            logger.info("Hybrid 파이프라인이 성공적으로 완료되었습니다!")
            return 0
        else:
            logger.error("Hybrid 파이프라인이 실패했습니다.")
            return 1
            
    except Exception as e:
        logger.error(f"Hybrid 파이프라인 실행 중 오류 발생: {str(e)}")
        return 1
